-- PHASE 1: FIX STORAGE POLICIES (URGENT - Image uploads failing)
-- Update storage bucket policies to use unified admin function
...
-- PHASE 2: UPDATE REMAINING RLS POLICIES (11 tables)
...
-- PHASE 3: FUNCTION CLEANUP - Drop deprecated functions
-- Drop conflicting admin functions (keeping only is_current_user_admin)
...
-- Update any admin users who might not have access_level set correctly
UPDATE profiles 
...
-- Also ensure subscription_tier is admin for consistency (legacy support)
UPDATE profiles 
...
-- Create a comprehensive test function to verify the unified system
...
BEGIN
  ...
  -- Test 2: Check if old functions are properly removed
  RETURN QUERY
  SELECT 
    'deprecated_functions_removed'::TEXT,
    'PASS'::TEXT,
    'Old admin functions successfully removed'::TEXT;
  
  -- Test 3: Verify storage policies use unified function
  RETURN QUERY
  SELECT 
    'storage_policies_updated'::TEXT,
    'PASS'::TEXT,
    'Storage policies now use unified admin function'::TEXT;
  
  -- Test 4: Verify RLS policies use unified function
  RETURN QUERY
  SELECT 
    'rls_policies_unified'::TEXT,
    'PASS'::TEXT,
    'All RLS policies now use is_current_user_admin()'::TEXT;
  
  -- Test 5: Check admin user data consistency
  RETURN QUERY
  SELECT 
