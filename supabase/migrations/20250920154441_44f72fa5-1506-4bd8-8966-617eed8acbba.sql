...
BEGIN
  ...
  IF NEW.status = 'active' AND OLD.status IS DISTINCT FROM 'active' THEN
    ...
    current_date := NEW.start_time::DATE;
    end_date := (NEW.start_time + (total_hours || ' hours')::INTERVAL)::DATE;
    
    -- Create slots for each day of the fast
    WHILE current_date <= end_date LOOP
      -- Calculate hours into fast for this day
      hours_passed := GREATEST(0, 
        EXTRACT(EPOCH FROM (current_date::TIMESTAMP WITH TIME ZONE + '1 day'::INTERVAL - NEW.start_time)) / 3600
      );
