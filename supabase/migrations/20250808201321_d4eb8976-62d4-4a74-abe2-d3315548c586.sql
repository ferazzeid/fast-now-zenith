-- First check what valid phases exist
SELECT DISTINCT phase FROM fasting_hours WHERE phase IS NOT NULL;

-- Update existing data to remove preparation references and add comprehensive data
...
SET 
  ...
  END,
  content_snippet = CONCAT('Hour ', hour, ': ', stage, ' — fat-burn rises, insulin stays low, and focus gets steadier.'),
  phase = CASE 
    WHEN phase = 'preparation' THEN 'post_absorptive'
    ELSE phase 
  END
