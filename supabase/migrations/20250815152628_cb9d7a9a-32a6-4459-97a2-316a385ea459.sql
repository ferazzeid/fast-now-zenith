...
-- Update OTP expiry to recommended 24 hours
UPDATE auth.config 
SET value = '86400' 
...
UPDATE auth.config 
SET value = 'true' 
WHERE parameter = 'PASSWORD_LEAK_PROTECTION';
...
ON storage.objects 
FOR ALL 
USING (bucket_id = 'website-images' AND 
  auth.uid() IS NOT NULL AND 
  EXISTS (
    SELECT 1 FROM public.profiles 
    WHERE user_id = auth.uid() 
...
ON storage.objects 
FOR ALL 
USING (bucket_id = 'background-images' AND 
  auth.uid() IS NOT NULL AND 
  EXISTS (
    SELECT 1 FROM public.profiles 
    WHERE user_id = auth.uid() 
