...
serve(async (req) => {
  ...
  try {
    ...
    logStep("Attempting to redeem coupon", { code: couponCode.substring(0, 3) + "..." });
    ...
    if (redemptionError) {
      logStep("Error redeeming coupon", { error: redemptionError.message });
      throw new Error(`Redemption failed: ${redemptionError.message}`);
    }

    const result = redemptionResult[0];
    logStep("Redemption result", result);
    ...
    return new Response(JSON.stringify({
      success: true,
      message: result.message,
      daysGranted: result.days_granted
    }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 200,
    });
