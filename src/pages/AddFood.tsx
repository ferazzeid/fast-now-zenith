  ...
  const handleAiEstimate = async () => {
    if (!name || !servingAmount) {
      toast({
        ...
        description: "Please enter food name and amount first",
        variant: "destructive"
      });
      return;
    }
      ...
      const { data: result, error } = await supabase.functions.invoke('chat-completion', {
        body: {
          messages: [
            ...
            { role: 'user', content: `For exactly ${servingAmount}g of ${name}, provide the total calories and carbs in grams for that specific amount. Return only valid JSON.` }
          ]
        }
      });
      ...
      try {
        ...
        if (typeof parsed.calories === 'number' && typeof parsed.carbs === 'number') {
          ...
          toast({
            ...
            description: `AI estimated ${parsed.calories} calories and ${parsed.carbs}g carbs for ${servingAmount}g`
          });
          return;
        }
      } catch {
          ...
          <div className="space-y-6">
                ...
                <div className="flex gap-2 items-center">
                  <Input
                    ...
                    placeholder={
                      ...
                      voiceProcessingState === 'analyzing' ? "Processing..." :
                      ...
                      <TooltipTrigger asChild>
                        <PremiumGate feature="AI Estimation" grayOutForFree={true}>
                          <Button
                            ...
                            className="h-9 w-9 p-0 flex items-center justify-center bg-primary hover:bg-primary/90 text-primary-foreground"
                          >
                            {isAiEstimating ? (
              ...
              <div className="pt-4">
                <Button
                  ...
                  className={`w-full h-10 !text-white ${!name.trim() ? 'bg-muted cursor-not-allowed' : 'bg-yellow-500 hover:bg-yellow-600'}`}
                >
                  {saving ? (
                    <>
