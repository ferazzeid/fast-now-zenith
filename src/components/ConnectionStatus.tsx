...
export const ConnectionStatus = () => {
  ...
  if (!isOnline) {
    return (
      <Alert className="border-destructive bg-destructive/10 fixed top-28 left-4 right-4 mx-auto max-w-sm z-50">
  ...
  if (!isConnected) {
    return (
      <Alert className="border-warning bg-warning/10 fixed top-28 left-4 right-4 mx-auto max-w-sm z-50">
        <WifiOff className="h-4 w-4" />
        <AlertDescription className="flex items-center justify-between">
          <div>
  ...
  if (hasQueuedOperations) {
    return (
      <Alert className="border-info bg-info/10 fixed top-28 left-4 right-4 mx-auto max-w-sm z-50">
