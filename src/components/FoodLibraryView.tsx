...
export const FoodLibraryView = ({ 
  ...
  const FoodCard = ({ food, isUserFood = true }: { food: UserFood | DefaultFood, isUserFood?: boolean }) => {
    ...
    return (
      <div 
        className="rounded-lg p-2 transition-all duration-200 border border-border/50 bg-background/50 cursor-pointer"
              ...
              {templateLoading ? (
                ...
                <div className="space-y-1 overflow-x-hidden">
                  {templateFoods.map((food) => (
                     <div key={food.id} className="rounded-lg p-2 transition-all duration-200 border border-border/50 bg-background/50">
                       <div className="flex items-center gap-2">
                         {/* Options Dropdown */}
